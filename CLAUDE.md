...
- `match_rounds` — added `round_number` (1-indexed) and `expires_at` (denormalized `started_at + question_duration_ms`, so no join is needed to render a per-question countdown). Unique constraint on `(match_id, round_number)`.
...
- `advance_match_round(p_match_id, p_current_round_number)` — the shared core of "what happens after a round resolves," called from both `submit_answer` (someone answered correctly) and `expire_round` (nobody did in time). Advisory-locks the match (`pg_advisory_xact_lock(hashtext(match_id)::bigint)`), checks whether `round_number + 1` already exists (no-op if so — this is what makes it safe for a correct-answer and an expiry to both fire near-simultaneously without double-advancing), and either creates the next round or, once `current_round_number >= question_count`, sets `matches.ended_at`.
- `submit_answer(p_round_id, p_guest_id, p_answer_index)` — same atomic-claim semantics as before (`UPDATE match_rounds SET winner_guest_id = ... WHERE winner_guest_id IS NULL`, so a race between N players only ever lets the first correct one claim it), but now also increments `match_results.score` for the winner and delegates round advancement to `advance_match_round`.
- `expire_round(p_round_id)` — new. Any connected client calls this once its local per-question countdown hits zero; the function re-verifies server-side (`now() >= expires_at` and still unclaimed) before calling `advance_match_round` — same "client triggers, server verifies" pattern the old bot fallback established. A round nobody answers correctly just advances with no one scoring it.
- **No client-side match clock at all anymore.** This is a deliberate fix for a real bug hit during manual testing (see git history / below): the old design compared a client's `Date.now()` against `matches.started_at`, which broke under `next dev`'s on-demand route compilation eating into the budget before the client even mounted. Now match-end is a pure server event — `matches.ended_at` flipping — broadcast to every client via the same realtime publication. The per-question countdown clients render is cosmetic (for the "Xs" UI text); `expire_round`'s own timestamp check is the actual authority, so a slow-mounting client can't desync the game state, only its own local display.
- `src/lib/match/actions.ts` — `getMatch` (now `questionCount`/`questionDurationMs`, no player columns), `getParticipants(matchId)` (the roster, for the scoreboard), `getCurrentRound` (now includes `roundNumber`/`expiresAt`), `submitAnswer`, `expireRound` (new), `triggerBotMove(botPlayerId, roundId)` (signature changed — takes the specific bot's player ID directly now that a match can have 0-3 bots, not a single implicit one; delay range retuned to 800-4000ms to fit inside the shorter ~5s question window, was 1.2-5.5s when tuned for a 15s match)
...
- `src/app/match/[matchId]/page.tsx` — now also fetches `getParticipants` and passes `questionCount` instead of `startedAt`/`isBotMatch`
- `src/components/match/MatchRoom.tsx` — rewritten for N players: subscribes to `match_rounds` INSERT/UPDATE, `match_results` UPDATE (live scores), and `matches` UPDATE (`ended_at` → match-end signal) via `postgres_changes`. Per-round effects: schedule one `expireRound` timeout per round (cleared/rescheduled on round change), fire `triggerBotMove` once per bot participant per round (both guarded by ref-based dedup sets keyed by round/participant so realtime re-renders can't double-fire them). Result screen sorts `match_results` by `score` desc for a ranked 1st-4th list.

Scores are no longer tracked client-side at all (the old `applyRoundWinner`/`initialWinCounts` reducer is gone) — `match_results.score` is the single source of truth, and the client just mirrors it via realtime UPDATE events.
...
- `src/lib/matchmaking/rpc.integration.test.ts` — the actual game rules live in Postgres functions (`matchmaking_try_form_match`, `matchmaking_bot_fallback`, `submit_answer`, `expire_round`, `advance_match_round`), which can't be meaningfully unit-tested without a real Postgres (no Docker in this environment, so no local Supabase stack for pgTAP-style testing). These tests call the RPCs directly against the live project via a raw `@supabase/supabase-js` client (NOT `src/lib/supabase/admin.ts` — that file does `import "server-only"`, which throws when imported outside Next's build pipeline; only Next's webpack config no-ops it) and clean up their own rows in `afterEach`. Cleanup order matters: `match_rounds` has an FK to `matches` with no `ON DELETE CASCADE`, so rounds must be deleted before their match or the delete silently fails via the ignored `error` field.
...
Steps 4-5-6 were reworked from strict 1v1 to up to 4 players per match, per-question countdowns, and a fixed question count (10) instead of a match-wide 15s clock — see the "4-player redesign" section above for the full data model and RPC rundown.

Verified end-to-end against the live project (via direct RPC calls, a raw script replicating `triggerBotMove`'s exact algorithm, and hitting a real production build's `/match/[matchId]` route): guest cookie/session flow works; `matchmaking_try_form_match` correctly waits until 4 are queued in the same category, doesn't mix categories, and produces 4 `match_results` rows + a real round 1; `matchmaking_bot_fallback` sweeps up every waiting human (not just the caller) after 15s and fills only the remaining seats with bots; `submit_answer` claims correctly (score increments, race losers get `claimed: false`, wrong answers don't score) and correctly ends the match instead of advancing once `question_count` rounds are done; `expire_round` correctly no-ops before `expires_at` and correctly advances (with no winner recorded) once it's genuinely passed. Production-build smoke test confirmed the match page renders all 4 participants, the question, and "Question 1 / 10" correctly, with `correct_answer_index` never appearing in the response.

30 automated tests now cover this (17 unit + 13 integration), all passing on a clean DB state.
